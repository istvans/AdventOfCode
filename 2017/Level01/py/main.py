...
printer's broken! We can't print the Naughty or Nice List!" By the time you make
 ...
 nothing else can print The List. Stand in this square, quick! There's no time
 to explain; if you can convince them to pay you in stars, you'll be able to--"
...
def main(args):
    ...
    if args.stdin:
        while string != 'q':
            string = input("give me a number (type 'q' to quit): ")
            if (is_int(string)):
                exec(args, string)
    else:
...
if __name__ == "__main__":
    ...
    group.add_argument("-s", "--stdin", help="read number from stdin",
                             action="store_true")
    parser.add_argument("-p", "--part", help="solution with part 1 or 2 algo",
